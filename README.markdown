...
var_dump($encoder->encodeNumber(42)); // string(4) "i42e"
...
There is also a convenience method simply called `encode` in the `PHP\BitTorrent\Encoder` class that can be used to encode all encodable variables (numbers, strings and arrays).
...
var_dump($decoder->decodeNumber('i42e')); // string(2) "42"
var_dump($decoder->decodeList('li1ei2ei3ee'); // array(3) { [0]=> string(1) "1" [1]=> string(1) "2" [2]=> string(1) "3" }
